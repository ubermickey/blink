...
readonly LANE_NAME="g4_scp_sftp"
readonly GATE_NAME="G4 Domain Gates"
readonly RESOURCE_GROUP="${RESOURCE_GROUP:-simulator_g4_scp_sftp}"
readonly WORKER_NAME="${WORKER_NAME:-gpt_5_3_codex__g4_file_provider}"
readonly COMMAND_DESC="xcodebuild -scheme BlinkQA test -only-testing:BlinkFileProviderTests/FileTranslatorFactoryTests"
...
status="pass"
coverage_mode="full"
notes="SCP/SFTP lane completed with isolated fixture root ${FIXTURE_REMOTE_ROOT}."
...
if ! run_logged "${FIXTURE_LOG}" "${IOS_SCRIPT_DIR}/fixture-up"; then
  if [[ "${allow_no_fixture}" -eq 1 ]]; then
    ...
    notes="Fixture unavailable; SCP/SFTP lane skipped with reduced coverage."
...
if [[ "${status}" == "pass" ]] && ! run_logged "${TEST_LOG}" \
  ...
  -only-testing:BlinkFileProviderTests/FileTranslatorFactoryTests \
  test; then
  status="fail"
  notes="SCP/SFTP test lane failed. See ${TEST_LOG}."
fi
