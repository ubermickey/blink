...
readonly LANE_NAME="g2_files"
readonly GATE_NAME="G2 Test"
readonly RESOURCE_GROUP="${RESOURCE_GROUP:-simulator_files}"
...
readonly COMMAND_DESC="xcodebuild build-for-testing + xcodebuild test-without-building (BlinkFilesTests, BlinkFileProviderTests)"
...
status="pass"
notes="Files shard completed with BlinkFilesTests and BlinkFileProviderTests."
simulator_id=""
...
if [[ "${status}" == "pass" ]]; then
  if ! run_logged "${TEST_LOG}" \
    ...
    -destination "id=${simulator_id}" \
    -derivedDataPath "${derived_data_dir}" \
    -only-testing:BlinkFilesTests \
    -only-testing:BlinkFileProviderTests; then
    status="fail"
    notes="Files shard failed. See ${TEST_LOG}."
  fi
fi
