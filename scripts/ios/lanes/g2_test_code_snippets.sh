...
readonly LANE_NAME="g2_code_snippets"
readonly GATE_NAME="G2 Test"
readonly RESOURCE_GROUP="${RESOURCE_GROUP:-simulator_code_snippets}"
...
readonly COMMAND_DESC="xcodebuild build-for-testing + xcodebuild test-without-building (BlinkCodeTests, BlinkSnippetsTests)"
...
status="pass"
notes="Code/Snippets shard completed with BlinkCodeTests and BlinkSnippetsTests."
simulator_id=""
...
if [[ "${status}" == "pass" ]]; then
  if ! run_logged "${TEST_LOG}" \
    ...
    -destination "id=${simulator_id}" \
    -derivedDataPath "${derived_data_dir}" \
    -only-testing:BlinkCodeTests \
    -only-testing:BlinkSnippetsTests; then
    status="fail"
    notes="Code/Snippets shard failed. See ${TEST_LOG}."
  fi
fi
