...
readonly LANE_NAME="g4_mosh_bootstrap"
...
readonly COMMAND_DESC="xcodebuild -scheme BlinkQA test -only-testing:BlinkTests/MoshBootstrapTests/testMoshBootstrap"
...
write_metadata() {
  ...
  python3 - "${METADATA_PATH}" "${LANE_NAME}" "${GATE_NAME}" "${status}" "${WORKER}" "${RESOURCE_GROUP}" "${COVERAGE_MODE}" "${notes}" "${started}" "${finished}" "${simulator_id}" "${fixture_host}" "${fixture_port}" "${TEST_LOG}" "${COMMAND_DESC}" <<'PY'
import json
...
export BLINK_TEST_USER="${BLINK_TEST_USER:-regular}"
export BLINK_TEST_PASSWORD="${BLINK_TEST_PASSWORD:-regular}"
export FIXTURE_ALLOW_NETWORK_DOWNLOADS="${FIXTURE_ALLOW_NETWORK_DOWNLOADS:-0}"
...
status="pass"
notes="Mosh bootstrap lane passed using fixture ${BLINK_TEST_HOST}:${FIXTURE_SSH_PORT}."
...
if ! run_logged "${TEST_LOG}" \
  ...
  -only-testing:BlinkTests/MoshBootstrapTests/testMoshBootstrap \
  test; then
  status="fail"
  notes="Mosh bootstrap lane failed. See ${TEST_LOG}."
fi
