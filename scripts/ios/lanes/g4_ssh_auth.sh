...
readonly LANE_NAME="g4_ssh_auth"
...
readonly COMMAND_DESC="xcodebuild -scheme BlinkQA test -only-testing:SSHTests/AuthTests"
...
status="pass"
notes="SSH auth lane passed using fixture ${BLINK_TEST_HOST}:${FIXTURE_SSH_PORT}."
...
if ! run_logged "${TEST_LOG}" \
  ...
  -only-testing:SSHTests/AuthTests \
  test; then
  status="fail"
  notes="SSH auth lane failed. See ${TEST_LOG}."
fi
