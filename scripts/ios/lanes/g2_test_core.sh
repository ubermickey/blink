...
readonly LANE_NAME="g2_core"
readonly GATE_NAME="G2 Test"
readonly RESOURCE_GROUP="${RESOURCE_GROUP:-simulator_core}"
...
readonly COMMAND_DESC="xcodebuild build-for-testing + xcodebuild test-without-building (BlinkTests, SSHTests, BlinkConfigTests)"
...
status="pass"
notes="Core shard completed with BlinkTests, SSHTests, and BlinkConfigTests."
simulator_id=""
...
if [[ "${status}" == "pass" ]]; then
  if ! run_logged "${TEST_LOG}" \
    ...
    -only-testing:SSHTests \
    -only-testing:BlinkConfigTests; then
    status="fail"
    notes="Core shard failed. See ${TEST_LOG}."
  fi
fi
